	     ...
	     ival_t *ival2, std::vector<tuple_t> &tuples)
{
  ...
  for (uint8_t i = offset; i < len; i+=3) {
    ...
    uint32_t low_addr = low_tmp/BUCKET_SIZE;
    uint32_t high_addr = high/BUCKET_SIZE;

    // update low
    uint32_t low_idx = low_tmp % BUCKET_SIZE;
    ...
    // update high
    uint32_t high_idx = high % BUCKET_SIZE;
    uint32_t high_new;
...
void decompressSeq(std::vector<read_t> &reads, char *ref, dcmp_fps &fps)
{
  ...
  // decompress
  for (uint32_t i = 0, k = 0; i < entries; i++) {
    int idx = 0;
