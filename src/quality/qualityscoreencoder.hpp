...
class QualityScoreEncoder {
  private:
    ...
    // Domain of inputs
    static const int BASE_ALPHABET_SIZE  = 41;
    static const int BASE_CHAR_OFFSET    = 33;
    static const int MAX_CONSEC_ZERO     = 20;
    static const int ALPHABET_SIZE       = BASE_ALPHABET_SIZE + MAX_CONSEC_ZERO + 1; 
    static const int SYMBOL_SIZE         = ALPHABET_SIZE * ALPHABET_SIZE * ALPHABET_SIZE;
    static const int TUPLE_SIZE = 3;    

    // Misc.
    uint64_t *frequency;                   // Frequencies to calculate encoding
  ...
  public:
    ...
    static void translate_symbol(std::vector<read_t>::iterator, std::vector<read_t>::iterator, std::vector<uint32_t>&, uint64_t*);
    void encode_symbol(uint32_t);
    void encode_flush(void);
    void decode_entry(read_t&);
    ~QualityScoreEncoder() { delete[] frequency; }
};
